# -*- encoding: utf-8 -*-
'''
# dns3 datasets: 10 seconds in total, random start if chunk_size_in_seconds < 10s
'''
...
def make_dataloader(is_train=True,
                    data_kwargs=None,
                    num_workers=4,
                    chunk_size_in_seconds=9,
                    batch_size=16):
    dataset = Datasets(**data_kwargs, chunk_size=chunk_size_in_seconds)
...
class Datasets(Dataset):
    ...
    def __init__(self, mix_scp=None, ref_scp=None, sr=16000, chunk_size_in_seconds=9):
        super(Datasets, self).__init__()
        self.mix_audio = handle_scp(mix_scp)
        self.ref_audio = [handle_scp(r) for r in ref_scp]
        self.sr = sr
        self.chunk_size = int(chunk_size_in_seconds * sr) if chunk_size_in_seconds is not None else None
